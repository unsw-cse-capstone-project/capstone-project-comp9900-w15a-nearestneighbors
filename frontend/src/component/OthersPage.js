...
// OtherPage, which is familiar with mypage
function OthersPage(props) {
  ...
  const addToBlackList = () => {
    api
      .get("/user_page/add_to_bannedlist/", {
        params: { banned_user_id: props.match.params.banned_user_id },
      })
      .then(({ data }) => {
        if (data.success) {
        ...
        >
          ...
          <div style={{ width: "1024px", margin: "0 auto", marginTop: "20px" }}>
            <h1>
              ...
              <Button type="link" onClick={() => addToBlackList()}>
                Add To BlackList
              </Button>
            </h1>
            <h2>Reviews</h2>
            <div style={{ maxHeight: "400px", overflowY: "auto" }}>
              {(userInfo.top_reviews || []).map(
                ({ movie_name, rating_number, review_comment }) => {
                  return (
                    <div key={movie_name}>
                      <Row>
                        <Col span={8}>
                  ...
                  return (
                    <Col key={name}>
                      <Card
                        ...
                        className={styles.wd}
                        cover={
                          <img alt="example" src={poster.replace("..", "")} />
                        }
                      >
