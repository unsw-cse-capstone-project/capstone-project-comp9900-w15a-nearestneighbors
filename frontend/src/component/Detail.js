...
/* the movie details page, which mainly displays movie information at the top of the design. The middle module displays the main reviews and adds a text box for writing reviews. 
   Finally, it displays similar movies. For commenters and movies on this page, corresponding jump services are provided.*/
const Editor = ({
  onChange,
  onSubmit,
  submitting,
  ...
  <>
    ...
    <Form.Item>
      <Rate
        ...
        value={desc.indexOf(Math.floor(valueRating)) + 1}
      />
...
function Detail(props) {
  ...
  const addToWishList = () => {
    api
      .get("/movies/detail/add_to_wishlist/", {
        params: { movie_id: props.match.params.mid },
      })
      .then(({ data }) => {
        if (data.success) {
    ...
    <>
      ...
      <div style={{ width: "1024px", margin: "0 auto" }}>
        ...
        <Row>
          ...
          {(detail.reviews || []).map(
            ({ user_name, review_comment, date, user_id, rating_number }) => {
              return (
                <Col key={user_name}>
                  <Comment
                    author={
                      <a onClick={() => gotoBlack(props, user_name, user_id)}>
                        {user_name}
                      </a>
                    }
                    avatar={
                    ...
                    content={<p>{review_comment}</p>}
                    datetime={
                      <div style={{ display: "flex" }}>
                        <Tooltip
                          title={moment(new Date(date)).format(
                            "YYYY-MM-DD HH:mm:ss"
                          )}
                        >
                        ...
                        <div style={{ marginLeft: "80px" }}>
                          <Rate
                            disabled
                            value={desc.indexOf(Math.floor(rating_number)) + 1}
                          />
                          <span className="ant-rate-text">
                            {Number(rating_number).toFixed(1)}
                          </span>
                        </div>
                      </div>
                    }
